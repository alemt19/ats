...
type DescripcionesOfertasAdminClientProps = {
  initialQuery: AdminGenericJobDescriptionsQueryParams
  initialData: AdminGenericJobDescriptionsResponse
}
  ...
  return (
    <>
      {Array.from({ length: rows }).map((_, index) => (
        <TableRow key={`description-row-skeleton-${index}`}>
          <TableCell>
...
export default function DescripcionesOfertasAdminClient({
  initialQuery,
  initialData,
}: DescripcionesOfertasAdminClientProps) {
  ...
  const updateUrl = React.useCallback(
    (updates: Partial<AdminGenericJobDescriptionsQueryParams>, resetPage = false) => {
      const next = normalizeAdminGenericJobDescriptionsQuery({
  ...
  const { data, isFetching } = useQuery<AdminGenericJobDescriptionsResponse>({
    queryKey: ["admin-generic-job-descriptions", query.search, query.page, query.pageSize],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/admin/descripciones-ofertas?${params.toString()}`)
      if (!response.ok) {
        throw new Error("No se pudieron cargar los puestos predefinidos")
      }

      return (await response.json()) as AdminGenericJobDescriptionsResponse
    },
    initialData,
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button asChild className="rounded-full sm:w-auto">
          <Link href="/admin/descripciones-ofertas/crear">
            <Plus className="mr-2 size-4" />
            Crear nuevo puesto predefinido
          </Link>
        </Button>
      </div>
      ...
      <Card className="gradient-border container mx-auto gap-4 rounded-3xl bg-card/90 shadow-soft @container">
        ...
        <CardContent className="space-y-4">
          {isFetching && !descriptions.length ? <TableSkeleton /> : null}
          ...
          <div className="overflow-hidden rounded-2xl border border-border/70 bg-background/80">
            <Table>
              ...
              <TableBody>
                      ...
                      <TableCell className="w-12 text-center">
                        <div className="flex items-center justify-center">
                          <Button variant="ghost" size="icon" asChild className="rounded-full">
                            <Link
                              href={`/admin/descripciones-ofertas/${encodeURIComponent(String(description.id))}`}
                              aria-label={`Ver descripción ${description.position}`}
                            >
                ...
                {!isFetching && !descriptions.length ? (
                  <TableRow>
                    <TableCell colSpan={3} className="text-muted-foreground py-10 text-center">
                      No se encontraron puestos con la búsqueda ingresada.
                    </TableCell>
                  </TableRow>
                ) : null}
              </TableBody>
